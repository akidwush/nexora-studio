...
## Fidelity validation and alpha semantics
The live editor is interactive and may have a different viewport or source runtime state than the export. To inspect **the frame that will be encoded**, use `Match export preview`: it runs a separate fresh sandbox at the **exact output resolution and frame timestamp** and displays its snapshot. MP4 frames are captured using the same code, and a selected reference frame is recaptured and compared using a **tight, bounded per-pixel RGBA budget**, including transparency (to accommodate minor browser raster rounding). A real H.264 decoder then compares the selected decoded video frame against the matte preview using bounded mean RGB and major-error fractions. On mismatch, fail closed rather than silently declaring a good export. Motion transforms/keyframes, text and inline SVG are covered by browser regression scenes. This validates tested browser/API combinations only, not every possible web feature.

H.264/MP4 does **not support transparency** in this pipeline. Transparent pixels are composited onto an explicit customizable opaque matte *identically* in preview and video; the untouched raw frame can be downloaded separately as a transparent lossless PNG. Data-embedded `@font-face` CSS is carried into snapshot SVG where supported by the browser. Remote fonts and media remain disabled by CSP. CSS filters, unsupported paint mechanisms or font rasterization differences may be rejected by visual parity verification. Always check output on the target browser/device before production use.
