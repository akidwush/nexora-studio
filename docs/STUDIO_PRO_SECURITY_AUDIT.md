...
| `index.html:34405-34560` | Third-party AI API keys were read and written in plaintext `localStorage` on the unsafe editor origin | Key panel and save are gated and former AI credential reads/writes removed; never supply dashboard keys/cookies to the dedicated editor origin. When the patched page actually loads, it proactively deletes legacy `studiopro_ai_key_*` plaintext records without ever logging key values and evicts known obsolete Workbox/CDN font caches. A stale installed service worker can still serve a previous app version before this runs, so operators must verify SW invalidation and deploy the editor on a fresh credential-free site. |
...
**Secondary media ingress follow-up:** The mandatory `scripts/harden-upstream-secondary-assets.mjs` also gates clip source replacement, audio-library replacement, individual sound effects, named and batch Markdown audio, and audio-library folder imports before any decoder or state mutation. The Chromium forged-MIME negative test is `tests/studio-pro-secondary-assets.browser.mjs`. `tests/studio-pro-ingress.browser.mjs` now checks malicious persisted names and IDs, nested URLs/code, and a valid local plain-text scene. This limits obvious ingress abuse but cannot replace full decoder safety checks and a typed schema review for public uploads.
