...
| `index.html:33788-33867`, `:33935-34021` | Persisted localStorage projects also feed `applyProject`; old projects may contain HTML/HIC clips or URLs | Validator on every `applyProject` before state mutation (220 clips/32 tracks/2 MiB JSON max, reject prototype keys, HTML/HIC/WAAPI and known active/remote clip sources); project gallery only accepts bounded base64 PNG/JPEG/WebP thumbnails. This is intentionally coarse and NOT a public JSON validator. |
| `index.html:27985-28225`, `:18079-18105`, `:23119-23135`, `:35628-35654` | Client media import, audio library, subtitles and reimport/folder assets | Front-door MIME+file-size limits added where audited: 20 MiB image (PNG/JPEG/WebP/GIF), 90 MiB audio, 250 MiB video, up to 1 MiB text subtitles; SVG not accepted as a direct image. Further magic-byte/decompression and every secondary clip-specific picker still require review. |
...
4. **Assets:** MIME alone is spoofable. Add content-signature validation,
   bounded dimension/decompression/audio decode, blob URL lifecycle cleanup,
   folder quota tests and safety reviews of all secondary upload pickers.
